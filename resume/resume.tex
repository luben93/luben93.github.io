% LaTeX resume using res.cls
\documentclass[margin]{res}
\usepackage{helvetica} % uses helvetica postscript font (download helvetica.sty)
\usepackage[utf8]{inputenc}
\usepackage{hyperref}
%\usepackage[headheight=0pt,headsep=0pt,bottom=0.1cm]{geometry}
%\addtolength{\topmargin}{-60pt}
%\addtolength{\bottommargin}{-60pt}
%\addtolength{\textheight}{120pt}

%\usepackage{newcent}   % uses new century schoolbook postscript font 
%\setlength{\textwidth}{5.1in} % set width of text portion

\begin{document}

% Center the name over the entire width of resume:
 \moveleft.5\hoffset\centerline{\large\bf Lucas Persson, 19930808-9136}
% Draw a horizontal line the whole width of resume:
 \moveleft\hoffset\vbox{\hrule width\resumewidth height 1pt}\smallskip
% address begins here
% Again, the address lines must be centered over entire width of resume:
 \moveleft.5\hoffset\centerline{+46730347518, \href{mailto:luben93@gmail.com}{luben93@gmail.com}, \href{https://www.github.com/luben93}{github.com/luben93}}
  \moveleft.5\hoffset\centerline{Studentbacken 25 1717}
 \moveleft.5\hoffset\centerline{Stockholm, 11557, Sweden}


\begin{resume}

\sloppy
\section{EDUCATION} 
%{\sl\textbf{University Engineering Degree, B.Sc. Engineering:}} Computer Enginerring \\
{\sl\textbf{Bachelor of Science in Engineering:}} Computer Engineering \\
                KTH Royal Institute of Technology, Stockholm Sweden, Ongoing \\
               %Estimated Graduation: May 2016 \\
                %Concentration: Computer Science \\
    {\sl\textbf{Certificates:}} Swedish Drivers Licence: Car \\
                        ESOL Cambridge Advanced English: Level 2, Grade C 

               
 
 
\section{COMPUTER \\ SKILLS} 
    {\sl\textbf{Languages High Level:}} Java, C, PHP, Python, Ruby on Rails, JavaScript, C\#
\\
    {\sl\textbf{Languages  Entry Level:}} Swift,  Scala, C++, Lua, Bash,  OpenGL, SDL, Visual Basic, VBA, HTML, CSS and Assembler
\\
            {\sl\textbf{ Operating Systems:}} Mac OS X, Linux, FreeBSD, Android and Windows 
\\
             {\sl\textbf{ Software \& Frameworks:}} Amazon AWS, Azure, Git, Docker, Terminal, Vim, Android API, Laravel, Java EE, ASP.NET, Node.js, Meteor.js, \LaTeX, PostgreSQL \& MySQL

 
\section{EXPERIENCE}
{\sl Bachelor Thesis: } \hfill Spring 2017\\
Indoor localization of hand-held Shopping Scanners, KTH
 \begin{itemize} \itemsep -2pt
                \item Sensor fusion, Java, Hardware Prototyping \hfill 
                \item \href{http://kth.diva-portal.org/smash/record.jsf?pid=diva2\%3A1108974}{TRITA-STH, 2017:38} \hfill 
                \end{itemize}
{\sl Fulltime Employment } \hfill From Oct 2016 \\
                Virtual Stores, Stockholm Sweden
                \begin{itemize} \itemsep -2pt
                \item Backend Developer, Java, C\# ASP.NET, PHP, C++, JavaScript\hfill 
                \end{itemize}
{\sl Halftime Employment } \hfill  Oct 2016 - Jun 2015 \\
                Zuccero.com, Stockholm Sweden
                 \begin{itemize}  \itemsep -2pt %reduce space between items
                 \item  Tech Projcet Manager, Ruby on Rails, Fullstack Developer and Sys Admin\hfill 
                \end{itemize}
{\sl Parttime Employment } \hfill  Aug 2015 - Jan 2015 \\
                THSKonsulter, Stockholm Sweden
                 \begin{itemize}  \itemsep -2pt %reduce space between items
                 \item  Consultant, PHP, Web Developer\hfill 
                \end{itemize}
    {\sl Internship } \hfill Spring - Summer 2014 \\
                If Skadeförsäkringar, Stockholm Sweden
                 \begin{itemize}  \itemsep -2pt %reduce space between items
                 \item Android Application Developent, mandatory course by KTH 
                 %$‘‘Mina saker’’, an app for indexing valueble objectes in your home, in case you need to prove what objects where covered by the home insurance.
                \end{itemize}

   {\sl Parttime Employment }  \\
               KTH Royal Institute of Technology, Stockholm Handen Sweden
               Teaching Assistant
                 \begin{itemize}  \itemsep -2pt %reduce space between items
                 \item   Operating Systems HI1025\hfill Spring 2015 and Spring 2014
                 \item  Intro to Programming HI1024\hfill Fall 2013
                \end{itemize}

                
     {\sl Summer Employment } Rolls-Royce PLC, Kristinehamn Sweden \hfill 2012 - 2011 
                 


\section{EXTRA CURRICULAR ACTIVITIES}
\begin{itemize}  
            \item Bartender at Björns ISH and Nymble NKM, KTH \hfill 2012 - 2014 
            \item IT-support \href{ishaninge.se}{IsHaninge.se}  ISH KTH \hfill 2014
            \item Elected {\it Chairman of the Board},  ISH KTH\hfill 2013 
            \item Chairman of the Board at a  political youth chapter \hfill 2011
\end{itemize}

\end{resume}
\end{document}



